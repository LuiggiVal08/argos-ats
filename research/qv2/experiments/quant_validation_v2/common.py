...
# ── label generators (leakage-free, consistent encoding) ─────────────
#
# CLASS ENCODING (GLOBAL STANDARD):
#   0 = SELL
#   1 = HOLD
#   2 = BUY
#
# LEAKAGE PREVENTION:
# - Labels MAY use future data (shift(-lookahead)) — that's the prediction target
# - Volatility for normalization MUST use ONLY past data (no future leakage)
# - Rolling statistics on future-shifted series are FORBIDDEN


def _compute_historical_volatility(close: pd.Series, window: int = 60) -> pd.Series:
    """
    Compute historical volatility using ONLY past returns.

    Uses close.pct_change() on historical data up to each point.
    NO future data leakage — rolling window only looks backward.
    """
...
def compute_vol_adj_returns(df: pd.DataFrame, lookahead: int = 5, vol_window: int = 60) -> pd.Series:
    """
    Compute volatility-adjusted forward returns WITHOUT lookahead bias.

    Returns:
        Series of volatility-adjusted returns for labeling.
        NaN for last `lookahead` rows (no future available).
    """
    close = df["close"].astype(float)

    # Future return for labeling (allowed — this IS the target)
    future_close = close.shift(-lookahead)
    raw_return = (future_close / close - 1.0) * 100.0

    # Historical volatility (NO leakage — uses only past returns up to each point)
    hist_vol = _compute_historical_volatility(close, window=vol_window)

    # Align: hist_vol at time t uses returns up to t
    # raw_return at time t uses close[t+lookahead] / close[t]
    # This is correct: we normalize the forward return by historical vol up to t
    return raw_return / hist_vol
...
def label_3class(
    ...
    """
    Generate 3-class labels with GLOBAL encoding:
        0 = SELL
        1 = HOLD
        2 = BUY

    Uses volatility-adjusted returns with NO lookahead bias in volatility computation.
    """
    adj = compute_vol_adj_returns(df, lookahead=lookahead, vol_window=vol_window)
    labels = np.full(len(adj), 1, dtype=int)  # Default: HOLD (1)
    labels[adj > threshold] = 2   # BUY (2)
    labels[adj < -threshold] = 0  # SELL (0)
    return labels
...
def label_binary(
    ...
    """
    Generate binary labels (SELL vs BUY, HOLD excluded).

    Returns:
        labels: 0=SELL, 1=BUY (HOLD masked out)
        valid_mask: True where label is SELL or BUY
    """
    adj = compute_vol_adj_returns(df, lookahead=lookahead, vol_window=vol_window)
    labels = np.full(len(adj), -1, dtype=int)
    labels[adj > threshold] = 1   # BUY (1)
    labels[adj < -threshold] = 0  # SELL (0)
    valid_mask = labels != -1
    return labels, valid_mask


def label_regression(
    df: pd.DataFrame,
    lookahead: int = 5,
    vol_window: int = 60,
) -> np.ndarray:
    """Generate continuous volatility-adjusted returns for regression."""
    return compute_vol_adj_returns(df, lookahead=lookahead, vol_window=vol_window).values.astype(np.float64)


# ── Label contract validation (for testing) ─────────────────────────

def validate_label_encoding(labels: np.ndarray) -> bool:
    """Verify labels use ONLY the global encoding: 0=SELL, 1=HOLD, 2=BUY."""
    unique = set(np.unique(labels[~np.isnan(labels)]).astype(int))
    return unique.issubset({0, 1, 2}) and len(unique) > 0


def verify_no_future_leakage_in_volatility(df: pd.DataFrame, lookahead: int = 5, vol_window: int = 60) -> bool:
    """
    Verify that volatility computation uses only past data.

    This is a design-time check — the implementation in compute_vol_adj_returns
    uses _compute_historical_volatility which only uses past returns.
    """
    # This is a contract verification — the actual check is in the implementation
    # We verify by ensuring _compute_historical_volatility doesn't use shift(-n)
